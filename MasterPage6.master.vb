...
Partial Class MasterPage5
    ...
    Protected Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Response.Redirect("thanks.html?name=" + txtName1.Text + "&email=" + txtEmailId1.Text)
        txtName1.Text = ""
        txtMobile1.Text = ""
        txtEmailId1.Text = ""
        'txtCollege.Text = ""
    ...
    Public Sub Footerinsert()
        ...
        If Phoneno.Text <> "" Then
            If Emailid.Text <> "" Then
                If Phoneno.Text <> "" Then
                    sql = ("insert into Admissions (EnquiryNo,StudentName,EmailID,StudentMobileNo,Course,ModeOfEnquiry,StatusCode,Session,FranchiseCode,BrandName,Admissiondate) values('" + enquiryno + "','" + "Student" + "','" + Emailid.Text + "','" + Phoneno.Text + "','" + "ThinkNEXT Brochure (Footer)" + "','" + "ThinkNEXT Brochure(Footer)" + "','" + "Enquiry" + "','" + ses + "','" + "TNK101" + "','" + "THINKNEXT TRAINING" + "',@Admissiondate)")
                    Dim cmd As New SqlCommand(sql, con)
                    cmd.Parameters.AddWithValue("@Admissiondate", Format(Now, "MM/dd/yyyy"))
                    cmd.ExecuteNonQuery()
                    cmd.Dispose()
                    con.Close()
                    varclass.SendLeads4(Phoneno.Text, "Student", Emailid.Text, "ThinkNEXT Brochure", "Download Brochure", "https://www.thinknexttraining.com/")
                    Emailid.Text = ""
                    Phoneno.Text = ""
    ...
    Protected Sub btnadd_Click2(sender As Object, e As EventArgs) Handles btnadd2.Click
        System.Threading.Thread.Sleep(0)
        stipendfreeintern()

    End Sub
    Public Sub stipendfreeintern()
        ...
        If con.State = ConnectionState.Closed Then
            con.Open()
        End If
        If StipendInternTxtPhone.Text <> "" Then
            If StipendInternTxtEmail.Text <> "" Then
                If StipendInternTxtPhone.Text <> "" Then
                    sql = ("insert into Admissions (EnquiryNo,StudentName,EmailID,StudentMobileNo,Course,ModeOfEnquiry,StatusCode,Session,FranchiseCode,BrandName,Admissiondate) values('" + enquiryno + "','" + StipendInternTxtName.Text + "','" + StipendInternTxtEmail.Text + "','" + StipendInternTxtPhone.Text + "','" + "ThinkNEXT Brochure (Footer)" + "','" + "ThinkNEXT Brochure(Footer)" + "','" + "Enquiry" + "','" + ses + "','" + "TNK101" + "','" + "THINKNEXT TRAINING" + "',@Admissiondate)")
                    Dim cmd As New SqlCommand(sql, con)
                    cmd.Parameters.AddWithValue("@Admissiondate", Format(Now, "MM/dd/yyyy"))
                    cmd.ExecuteNonQuery()
                    cmd.Dispose()
                    con.Close()
                    varclass.SendLeads4(StipendInternTxtPhone.Text, StipendInternTxtName.Text, StipendInternTxtEmail.Text, "Stipend Based/Free Internship", "Footer Form", "https://www.thinknexttraining.com/")
                    StipendInternTxtName.Text = ""
                    StipendInternTxtEmail.Text = ""
                    StipendInternTxtPhone.Text = ""
