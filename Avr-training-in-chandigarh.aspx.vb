...
Partial Class Avr_training_in_chandigarh
        ...
        If txtName.Text <> "" Then
            If txtEmailId.Text <> "" Then
                If txtMobile.Text <> "" Then
                    sql = ("insert into Admissions (EnquiryNo,StudentName,EmailID,StudentMobileNo,Course,ModeOfEnquiry,StatusCode,Session,FranchiseCode,BrandName,Admissiondate) values('" + enquiryno + "','" + txtName.Text + "','" + txtEmailId.Text + "','" + txtMobile.Text + "','" + "AVR  " + "','" + "Arrange a Call Back" + "','" + "Enquiry" + "','" + ses + "','" + "TNK101" + "','" + "THINKNEXT TRAINING" + "',@Admissiondate)")
                    ...
                    varclass.SendLeads(txtMobile.Text, txtName.Text, txtEmailId.Text, "AVR  ", "Arrange a Call Back", "https://www.thinknexttraining.com/Avr-training-in-chandigarh.aspx")
                    lblmsg.Visible = True
                    lblmsg.ForeColor = Drawing.Color.Red
                    lblmsg.Text = "Thanks for your Enquiry at ThinkNEXT Technologies Pvt. Ltd."
                    txtName.Text = ""
    ...
    Public Sub SendEmailbroucher(ByVal enquiryno As String)
        ...
        StrContent = StrContent.Replace("[Course]", "AVR ")
    ...
    Protected Sub Btndemo_Click(sender As Object, e As EventArgs) Handles Btndemo.Click
        ...
        If txtdemoEmail.Text <> email Then
            ...
            varclass.SendLeads(txtdemoMblno.Text, txtdemoName.Text, txtdemoEmail.Text, "AVR  ", "Demo Class", "https://www.thinknexttraining.com/Avr-training-in-chandigarh.aspx")
            txtdemoName.Text = ""
            txtdemoMblno.Text = ""
            txtdemoEmail.Text = ""
            txtdemoMsg.Text = ""
    ...
    Sub AddStudentScholarshipDetail()
        ...
        cmd.Parameters.AddWithValue("@Duration", "AVR   Demo Class")
        cmd.Parameters.AddWithValue("@Course", "AVR  ")
        'cmd.Parameters.AddWithValue("@CollegeName", )
        cmd.Parameters.AddWithValue("@Comment", "AVR   Demo Class")
    ...
    Sub SendEmailReg()
        ...
        Try
            ...
            Dim mail As New _
                MailMessage("ThinkNEXT Technologies <training@thinknext.co.in>",
                            txtdemoEmail.Text, Trim("Free Demo Class AVR "),
                            Trim(StrContent.ToString()))
            mail.IsBodyHtml = True
            mail.Priority = MailPriority.High
            ' DEFINE HOTMAIL SMTP MAIL SERVER ALONG WITH A PORT (587 IN THIS CASE)
        ...
        If NameSyl.Text <> "" Then
            If EmailidSyl.Text <> "" Then
                If PhonenoSyl.Text <> "" Then
                    sql = ("insert into Admissions (EnquiryNo,StudentName,EmailID,StudentMobileNo,Course,ModeOfEnquiry,StatusCode,Session,FranchiseCode,BrandName,Admissiondate) values('" + enquiryno + "','" + NameSyl.Text + "','" + EmailidSyl.Text + "','" + PhonenoSyl.Text + "','" + "AVR  " + "','" + "Download Syllabus" + "','" + "Enquiry" + "','" + ses + "','" + "TNK101" + "','" + "THINKNEXT TRAINING" + "',@Admissiondate)")
                    Dim cmd As New SqlCommand(sql, con)
                    cmd.Parameters.AddWithValue("@Admissiondate", Format(Now, "MM/dd/yyyy"))
                    cmd.ExecuteNonQuery()
                    cmd.Dispose()
                    con.Close()
                    varclass.SendLeads4(PhonenoSyl.Text, NameSyl.Text, EmailidSyl.Text, "AVR  ", "Download Syllabus", "https://www.thinknexttraining.com/Avr-training-in-chandigarh.aspx")
                    NameSyl.Text = ""
                    EmailidSyl.Text = ""
                    PhonenoSyl.Text = ""
                    Response.Redirect("download-syllabus-thanks.aspx?data=AVR -training")
