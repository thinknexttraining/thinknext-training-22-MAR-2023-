...
Partial Class summer_internship_jackpot_offer
        ...
        If PersonName.Text <> "" Then
            If PersonEmail.Text <> "" Then
                If MobileNo.Text <> "" Then
                    sql = ("insert into Admissions (EnquiryNo,StudentName,EmailID,StudentMobileNo,Course,ModeOfEnquiry,StatusCode,Session,FranchiseCode,BrandName,Admissiondate) values('" + enquiryno + "','" + PersonName.Text + "','" + PersonEmail.Text + "','" + MobileNo.Text + "','" + PersonCourse.Text + "','" + "Summer Internship Jackpot Offer" + "','" + "Enquiry" + "','" + ses + "','" + "TNK101" + "','" + "THINKNEXT TRAINING" + "',@Admissiondate)")
                    Dim cmd As New SqlCommand(sql, con)
                    cmd.Parameters.AddWithValue("@Admissiondate", Format(Now, "MM/dd/yyyy"))
                    cmd.ExecuteNonQuery()
                    cmd.Dispose()
                    con.Close()
                    varclass.SendLeads(MobileNo.Text, PersonName.Text, PersonEmail.Text, PersonCourse.Text, "Summer Internship Jackpot Offer", "https://thinknexttraining.com/lp/summer-internship-jackpot-offer.aspx")
                    PersonName.Text = ""
                    PersonEmail.Text = ""
                    MobileNo.Text = ""
                    PersonCourse.Text = ""
                    Response.Redirect("https://www.thinknexttraining.com/lp/summer-internship-offers.html")
