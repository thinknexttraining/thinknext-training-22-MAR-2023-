...
Partial Class job_oriented_courses_12th_lp
        ...
        If PersonName.Text <> "" Then
            If PersonEmail.Text <> "" Then
                If MobileNo.Text <> "" Then
                    sql = ("insert into Admissions (EnquiryNo,StudentName,EmailID,StudentMobileNo,Course,ModeOfEnquiry,StatusCode,Session,FranchiseCode,BrandName,Admissiondate) values('" + enquiryno + "','" + PersonName.Text + "','" + PersonEmail.Text + "','" + MobileNo.Text + "','" + PersonCourse.Text + "','" + "Job Oriented Course after 12th" + "','" + "Enquiry" + "','" + ses + "','" + "TNK101" + "','" + "THINKNEXT TRAINING" + "',@Admissiondate)")
                    Dim cmd As New SqlCommand(sql, con)
                    cmd.Parameters.AddWithValue("@Admissiondate", Format(Now, "MM/dd/yyyy"))
                    cmd.ExecuteNonQuery()
                    cmd.Dispose()
                    con.Close()
                    varclass.SendLeads(MobileNo.Text, PersonName.Text, PersonEmail.Text, PersonCourse.Text, "Job Oriented Courses after 12th", "https://thinknexttraining.com/lp/job-oriented-courses-after-12th.aspx")
                    PersonName.Text = ""
                    PersonEmail.Text = ""
                    MobileNo.Text = ""
                    PersonCourse.Text = ""
                    Response.Redirect("https://www.thinknexttraining.com/lp/job-oriented-courses.html")
