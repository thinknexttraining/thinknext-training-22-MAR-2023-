...
Partial Class threeds_max_online_course
        ...
        If txtName.Text <> "" Then
            If txtEmailId.Text <> "" Then
                If txtMobile.Text <> "" Then
                    sql = ("insert into Admissions (EnquiryNo,StudentName,EmailID,StudentMobileNo,Course,ModeOfEnquiry,StatusCode,Session,FranchiseCode,BrandName,Admissiondate) values('" + enquiryno + "','" + txtName.Text + "','" + txtEmailId.Text + "','" + txtMobile.Text + "','" + "3DS Max" + "','" + "Arrange a Call Back" + "','" + "Enquiry" + "','" + ses + "','" + "TNK101" + "','" + "THINKNEXT TRAINING" + "',@Admissiondate)")
                    ...
                    varclass.SendLeads(txtMobile.Text, txtName.Text, txtEmailId.Text, "3DS Max", "Arrange a Call Back", "https://www.thinknexttraining.com/3ds-max-online-course.aspx")
                    lblmsg.Visible = True
                    lblmsg.ForeColor = Drawing.Color.Red
                    lblmsg.Text = "Thanks for your Enquiry at ThinkNEXT Technologies Pvt. Ltd."
                    txtName.Text = ""
    ...
    Public Sub SendEmailbroucher(ByVal enquiryno As String)
        ...
        StrContent = StrContent.Replace("[Course]", "3DS Max")
    ...
    Protected Sub Btndemo_Click(sender As Object, e As EventArgs) Handles Btndemo.Click
        ...
        If txtdemoEmail.Text <> email Then
            ...
            varclass.SendLeads(txtdemoMblno.Text, txtdemoName.Text, txtdemoEmail.Text, "3DS Max Demo", "Demo Class", "https://www.thinknexttraining.com/3ds-max-online-course.aspx")
            txtdemoName.Text = ""
            txtdemoMblno.Text = ""
            txtdemoEmail.Text = ""
            txtdemoMsg.Text = ""
    ...
    Sub AddStudentScholarshipDetail()
        ...
        cmd.Parameters.AddWithValue("@Duration", "3DS Max Demo Class")
        cmd.Parameters.AddWithValue("@Course", "3DS Max")
        'cmd.Parameters.AddWithValue("@CollegeName", )
        cmd.Parameters.AddWithValue("@Comment", "3DS Max Demo Class")
    ...
    Sub SendEmailReg()
        ...
        Try
            ...
            Dim mail As New _
                MailMessage("ThinkNEXT Technologies <training@thinknext.co.in>",
                            txtdemoEmail.Text, Trim("Free Demo Class 3DS Max"),
                            Trim(StrContent.ToString()))
            mail.IsBodyHtml = True
            mail.Priority = MailPriority.High
            ' DEFINE HOTMAIL SMTP MAIL SERVER ALONG WITH A PORT (587 IN THIS CASE)
        ...
        If NameSyl.Text <> "" Then
            If EmailidSyl.Text <> "" Then
                If PhonenoSyl.Text <> "" Then
                    sql = ("insert into Admissions (EnquiryNo,StudentName,EmailID,StudentMobileNo,Course,ModeOfEnquiry,StatusCode,Session,FranchiseCode,BrandName,Admissiondate) values('" + enquiryno + "','" + NameSyl.Text + "','" + EmailidSyl.Text + "','" + PhonenoSyl.Text + "','" + "3DS Max" + "','" + "Download Syllabus" + "','" + "Enquiry" + "','" + ses + "','" + "TNK101" + "','" + "THINKNEXT TRAINING" + "',@Admissiondate)")
                    Dim cmd As New SqlCommand(sql, con)
                    cmd.Parameters.AddWithValue("@Admissiondate", Format(Now, "MM/dd/yyyy"))
                    cmd.ExecuteNonQuery()
                    cmd.Dispose()
                    con.Close()
                    varclass.SendLeads4(PhonenoSyl.Text, NameSyl.Text, EmailidSyl.Text, "3DS Max", "Download Syllabus", "https://www.thinknexttraining.com/3ds-max-online-course.aspx")
                    NameSyl.Text = ""
                    EmailidSyl.Text = ""
                    PhonenoSyl.Text = ""
