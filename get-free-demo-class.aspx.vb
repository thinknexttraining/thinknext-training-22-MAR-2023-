...
Partial Class get_free_class
    ...
    Sub AddStudentDetail()
        ...
        If txtName.Text <> "" Then
            ...
            cmd.Parameters.AddWithValue("@ModeOfEnquiry", "Demo Class Enquiry")
            ...
            cmd.Parameters.AddWithValue("@Comment", "www.thinknexttraining.com/get-free-demo-class.aspx")
    ...
    Protected Sub BtnCallBack_Click(sender As Object, e As EventArgs) Handles BtnCallBack.Click

        If TxtCbName.Text = "" Then
            LblCbMsg.Visible = True
            LblCbMsg.ForeColor = Drawing.Color.Red
            LblCbMsg.Text = "Name field cannot be blank."
            Exit Sub
        End If
        If TxtCbPhone.Text = "" Then
            LblCbMsg.Visible = True
            LblCbMsg.ForeColor = Drawing.Color.Red
            LblCbMsg.Text = "Mobile Number field cannot be blank."
            Exit Sub
        End If
        If TxtCbEmail.Text = "" Then
            LblCbMsg.Visible = True
            LblCbMsg.ForeColor = Drawing.Color.Red
            LblCbMsg.Text = "Email ID field cannot be blank."
            Exit Sub
        End If
        'If txtCourses.Text = "" Then
        '    lblmsg.Visible = True
        ...
        LblCbMsg.Text = "Thanks for your Enquiry at ThinkNEXT Technologies Pvt. Ltd."
        'varclass.SendLeads(txtMobile.Text, txtName.Text, txtEmailId.Text, txtCourses.Text, "Arrange a Call Back", "https://www.thinknexttraining.com/")
        Response.Redirect("thanks.html")
    ...
    Sub AddCbStudentDetail()
        ...
        Dim objTextInfo As TextInfo = cultureInfo.TextInfo
        If TxtCbName.Text <> "" Then
            TxtCbName.Text = objTextInfo.ToTitleCase(TxtCbName.Text)
            'txtCourses.Text = objTextInfo.ToTitleCase(txtCourses.Text)
            sql = "insert into Admissions( Session,EnquiryNo, Admissiondate,StudentName,StudentMobileNo,EmailID,Course,Comment,ModeOfEnquiry,StatusCode,FranchiseCode,BrandName,EnquiryStatus)values(@Session,@EnquiryNo,@Admissiondate,@StudentName,@StudentMobileNo,@EmailID,@Course,@Comment,@ModeOfEnquiry,@StatusCode,@FranchiseCode,@BrandName,@EnquiryStatus)"
            Dim cmd As New SqlCommand(sql, con)
            Dim enquiryno As String
            ...
            cmd.Parameters.AddWithValue("@EnquiryNo", enquiryno)
            cmd.Parameters.AddWithValue("@Admissiondate", Format(Now, "MM/dd/yyyy"))
            cmd.Parameters.AddWithValue("@StudentName", TxtCbName.Text)
            cmd.Parameters.AddWithValue("@StudentMobileNo", TxtCbPhone.Text)
            cmd.Parameters.AddWithValue("@ModeOfEnquiry", "Call Back Enquiry")
            cmd.Parameters.AddWithValue("@EmailID", TxtCbEmail.Text)
            ' cmd.Parameters.AddWithValue("@Duration", ddlTrainingType.Text)
            '  cmd.Parameters.AddWithValue("@CollegeName", txtCollege.Text)
